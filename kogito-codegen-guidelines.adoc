...
TIP: Rule of Thumb: You should generate the code that you would write by hand, if there was no codegen step. 
...
1. Move shared, non-dynamic code to non-generated classes (libraries or inheritable classes) 
2. Specialize runtime-specific annotation/behavior (Quarkus vs. Spring) using templates if possible (inherit or call libary methods) 
...
These templates are "customized" using an *Annotator*. We provide an Annotator for Spring and CDI (Quarkus). Annotators decorate methods of a template using framework-specific annotations.  

These templates are largely the same for all frameworks we support. However, If a template is largely the same for all frameworks, you should consider *refactoring to a common shared class/library*.
...
You should avoid using conditional logic in code generation procedures.

Good code-generation procedures should not conditionally decide whether a piece of code should be generated "inline"; code generation should strive to push decision logic at the beginning, and then only instantiate the correct template.
